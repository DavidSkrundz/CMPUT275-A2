...
class GUI(LayeredUpdates):
	...
	def load_level(self, filename):
		...
		#
		#
		#	New code to populate powerups
		#
		#
		while line.find("POWERUPS START") < 0:
			line = map_file.readline()
			if line == "":
				raise Exception ("Expected powerups definitions")
		line = map_file.readline()

		while line.find("POWERUPS END") < 0:
			if line == "":
				raise Exception ("Expected end of unit definitions")
			line = line.rstrip()
			line = line.split(' ')
			...
			if not powerup_name in powerup.powerup_types:
				raise Exception("No powerup of name {} found!".format(powerup_name))
			new_powerup = powerup.powerup_types[powerup_name](tile_x = powerup_x, tile_y = powerup_y)

			# Add the unit to the update group and set its display rect
			self.update_unit_rect(new_powerup)

			line = map_file.readline()
			if line == "":
				raise Exception ("Expected end of powerups definitions")
